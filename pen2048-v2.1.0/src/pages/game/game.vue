<template>
  <div class="page" :class="dark ? 'page-d' : ''">
    <!-- 顶栏：得分 / 最佳 / 作弊（模式切换已移到左侧竖排） -->
    <div class="topbar">
      <div class="topbar-spacer" />
      <text class="topbar-score" :class="dark ? 'topbar-score-d' : ''">得分 {{ activeGame.score }}</text>
      <text class="topbar-score" :class="dark ? 'topbar-score-d' : ''">最佳 {{ best[mode] }}</text>
      <!-- 作弊开关框：默认绿（未开启），开启后红 -->
      <div class="tbtn cheat-toggle" :class="dark ? 'tbtn-d' : ''" @click="toggleCheat">
        <text class="tbtn-label" :class="dark ? 'tbtn-label-d' : ''">作弊</text>
    ...
    <div class="body">
      ...
      <div class="mode-rail">
        <div
          ...
          :key="s"
          class="mbtn"
          :class="(mode === s ? 'mbtn-active ' : 'mbtn ') + (dark && mode !== s ? 'mbtn-d ' : '')"
          @click="switchMode(s)"
        ><text class="mbtn-label" :class="(mode === s ? 'mbtn-label-active ' : 'mbtn-label ') + (dark ? 'mbtn-label-d ' : '')">{{ s }}×{{ s }}</text></div>
      </div>

      <!-- 棋盘区：滑动控制，作弊时点格子选中 -->
      <div class="board-area" @touchstart="onTouchStart" @touchend="onTouchEnd">
        <Board :grid="activeGame.grid" :size="activeGame.size" :sel="activeGame.cheat ? selCell : null" :dark="dark" @celltap="onCellTap" />
      </div>

      <!-- 右侧控制区：菜单按钮（作弊框下方）+ 作弊面板 / 菜单面板 -->
      <div class="control-panel">
        <!-- 菜单按钮：右对齐顶栏作弊框，展开/收起功能菜单 -->
        <div class="menu-btn" @click="menuOpen = !menuOpen">
          <text class="menu-btn-label">{{ menuOpen ? '收起' : '菜单' }}</text>
        </div>
        ...
        <!-- 功能菜单（原右侧功能按钮移入） -->
        <scroller v-else-if="menuOpen" scroll-y show-scrollbar="false" class="menu-scroller">
          <div v-if="overlay === 'won'" class="menu-item" @click="menuDo('continue')"><text class="menu-item-label">继续挑战</text></div>
          <div class="menu-item" @click="menuDo('restart')"><text class="menu-item-label">再来一局</text></div>
          <div class="menu-item" @click="menuDo('restart')"><text class="menu-item-label">重新开始</text></div>
          <div class="menu-item" @click="menuDo('save')"><text class="menu-item-label">存档界面</text></div>
          <div class="menu-item menu-item-plain" :class="dark ? 'menu-item-plain-d' : ''" @click="menuDo('menu')"><text class="menu-item-label-plain" :class="dark ? 'menu-item-label-plain-d' : ''">标题画面</text></div>
        </scroller>

        <text v-if="overlay !== 'none'" class="board-status" :class="dark ? 'board-status-d' : ''">{{ overlay === 'won' ? '达成 2048！' : '游戏结束' }}</text>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      /* 功能菜单展开状态（菜单按钮在作弊框下方，点击展开右侧菜单面板） */
      menuOpen: false,
      /* 改值面板的数值按钮分组 */
      valRows: [
        [2, 4, 8, 16],
...
/* 菜单按钮：与顶栏作弊框同宽（47vh）同右缘（13vh）对齐 */
.menu-btn {
  align-self: flex-end;
  width: 47vh;
  height: 10vh;
